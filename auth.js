...
onAuthStateChanged(auth, async user => {
  ...
  if (user) {
    ...
    if (dropdown) {
      ...
      userItem.append(document.createTextNode(`👤 ${username}`));
      if (titleInfo) {
        const titleBadge = document.createElement("span");
        titleBadge.textContent = titleInfo.label;
        titleBadge.style.cssText =
          `font-size:10px;font-weight:700;color:${titleInfo.color};letter-spacing:.5px;margin-left:4px;`;
        userItem.append(titleBadge);
      }

      const dividerOne = document.createElement("div");
      dividerOne.className = "cog-divider";
      const settingsItem = document.createElement("div");
      settingsItem.className = "cog-item";
      settingsItem.textContent = "Settings";
      settingsItem.onclick = () => { window.location.href = "settings.html"; };
      const dividerTwo = document.createElement("div");
      dividerTwo.className = "cog-divider";
      const signOutItem = document.createElement("div");
      signOutItem.className = "cog-item";
      signOutItem.id = "signOutBtn";
      signOutItem.textContent = "Sign Out";
      signOutItem.onclick = () => signOut(auth).then(() => location.reload());
